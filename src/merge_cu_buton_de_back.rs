...
async fn main(_spawner: Spawner) {
    ...
    loop {
        // ==== BACKGROUND AUDIO ENGINE (Runs regardless of screen) ====
        if let Some(ref file) = current_file {
            ...
            if FILE_DONE.load(Ordering::Relaxed) {
                ...
                // Daca suntem pe ecranul player, facem refresh la nume
...
fn draw_player<D: DrawTarget<Color = Rgb565>>(display: &mut D, filename: &ShortFileName, cursor: usize, is_playing: bool) {
    display.clear(Rgb565::BLACK).ok();
    Text::new("Now Playing", Point::new(80, 30), MonoTextStyle::new(&FONT_9X15_BOLD, Rgb565::CSS_GOLD)).draw(display).ok();
    let name_str = core::str::from_utf8(filename.base_name()).unwrap_or("???");
    let display_str: String<20> = String::try_from(name_str).unwrap_or_default();
    Text::new(&display_str, Point::new(10, 120), MonoTextStyle::new(&FONT_9X15, Rgb565::WHITE)).draw(display).ok();
    
    let labels = ["[ << ]", "[ || ]", "[ >> ]"];
    let x_positions = [10i32, 95, 190];
    for (i, (label, x)) in labels.iter().zip(x_positions.iter()).enumerate() {
        if i == cursor { Rectangle::new(Point::new(*x - 3, 185), Size::new(75, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok(); }
        let color = if i == cursor { Rgb565::WHITE } else { Rgb565::CSS_STEEL_BLUE };
        Text::new(label, Point::new(*x, 200), MonoTextStyle::new(&FONT_9X15, color)).draw(display).ok();
    }

    // Butonul BACK virtual sub controale
    if cursor == 3 { Rectangle::new(Point::new(100, 235), Size::new(100, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok(); }
    let back_color = if cursor == 3 { Rgb565::WHITE } else { Rgb565::CSS_STEEL_BLUE };
    Text::new("[ BACK ]", Point::new(110, 250), MonoTextStyle::new(&FONT_9X15, back_color)).draw(display).ok();
...
fn update_player_cursor<D: DrawTarget<Color = Rgb565>>(display: &mut D, old_cursor: usize, new_cursor: usize) {
    ...
    // Stergere highlight vechi
    if old_cursor < 3 {
        let x = x_positions[old_cursor];
        Rectangle::new(Point::new(x - 3, 185), Size::new(75, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
        Text::new(labels[old_cursor], Point::new(x, 200), MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_STEEL_BLUE)).draw(display).ok();
    } else {
        Rectangle::new(Point::new(100, 235), Size::new(100, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
        Text::new("[ BACK ]", Point::new(110, 250), MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_STEEL_BLUE)).draw(display).ok();
    }

    // Desenare highlight nou
    if new_cursor < 3 {
        ...
        Text::new(labels[new_cursor], Point::new(x, 200), MonoTextStyle::new(&FONT_9X15, Rgb565::WHITE)).draw(display).ok();
    } else {
        Rectangle::new(Point::new(100, 235), Size::new(100, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
        Text::new("[ BACK ]", Point::new(110, 250), MonoTextStyle::new(&FONT_9X15, Rgb565::WHITE)).draw(display).ok();
    }
}

fn update_player_status<D: DrawTarget<Color = Rgb565>>(display: &mut D, is_playing: bool) {
    Rectangle::new(Point::new(80, 145), Size::new(120, 20)).into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
    let status = if is_playing { ">> Playing" } else { "|| Paused " };
    Text::new(status, Point::new(80, 160), MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_LIME_GREEN)).draw(display).ok();
}