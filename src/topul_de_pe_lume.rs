...
// === VARIABILA GLOBALA PENTRU COMUNICAREA ASINCRONA ===
...
// ==== TASK SEPARAT PENTRU HAPTIC FEEDBACK ====
#[embassy_executor::task]
async fn haptic_task(mut motor: Output<'static>) {
    loop {
        if HAPTIC_TRIGGER.load(Ordering::Relaxed) {
            HAPTIC_TRIGGER.store(false, Ordering::Relaxed); // Resetam trigger-ul
            
            motor.set_high();
            Timer::after_millis(80).await; // 80ms creeaza o vibratie lunga si puternica
            motor.set_low();
        }
        // Poll la fiecare 5ms (insesizabil pentru om, nu consuma procesor aiurea)
...
async fn main(_spawner: Spawner) {
    ...
    // Display SPI1 - VITEZA MARITA la 32MHz
    ...
    // SD SPI2 - VITEZA MARITA la 24MHz
    ...
    // Initializam motorul si pornim task-ul asincron in fundal
    ...
    volume_mgr.iterate_dir(root_dir, |entry| {
        if !entry.attributes.is_directory() && !entry.attributes.is_hidden() {
            ...
            if ext == b"WAV" || ext == b"wav" { let _ = file_names.push(entry.name.clone()); }
        }
    }).unwrap();
    ...
    loop {
        ...
        if app_state == AppState::Playing {
            ...
            if FILE_DONE.load(Ordering::Relaxed) {
                ...
                draw_player(&mut display, &file_names[current_idx], player_cursor, true);
            }
        }

        // ==== TOUCH HANDLING: IPOD CLICK-WHEEL + HAPTIC FEEDBACK ====
        let mut buf = [0u8; 2];
        if i2c.write_read(0x5A, &[0x00], &mut buf).await.is_ok() {
            let touched = (buf[0] as u16) | ((buf[1] as u16) << 8);
            let wheel_touch = touched & 0xFF; // Ne uitam doar la electrozii 0..7
            
            // Gasim pe ce electrod e degetul acum
            let mut current_pos: Option<i8> = None;
            for i in 0..8 {
                if wheel_touch & (1 << i) != 0 {
                    current_pos = Some(i as i8);
                    break; 
                }
            }

            // Daca degetul e pe roata
            if let Some(c_pos) = current_pos {
                // Daca degetul era si inainte pe roata (putem compara)
                if let Some(l_pos) = last_wheel_pos {
                    // Diferenta circulara. Adaugam 8 inainte de modulo pt numere negative.
                    let diff = (c_pos - l_pos + 8) % 8;
                    
                    let mut move_down = false; // Scroll Dreapta / Jos
                    let mut move_up = false;   // Scroll Stanga / Sus

                    // Daca am avansat cu 1 sau 2 pozitii in sens orar (ex: 0->1, 7->0, 6->0)
                    if diff == 1 || diff == 2 {
                        move_down = true;
                    } 
                    // Daca am mers invers cu 1 sau 2 pozitii (ex: 1->0, 0->7, 0->6)
                    else if diff == 7 || diff == 6 {
                        move_up = true;
                    }

                    if move_up {
                        // === HAPTIC FEEDBACK ASINCRON ===
                        ...
                        match app_state {
                            AppState::Menu => {
                                if selected > 0 { 
                                    let old = selected;
                                    selected -= 1; 
                                    update_menu_selection(&mut display, &file_names, old, selected);
                                }
                            }
                            AppState::Playing => {
                                if player_cursor > 0 { 
                                    let old = player_cursor;
                                    player_cursor -= 1; 
                                    update_player_cursor(&mut display, old, player_cursor);
                                }
                            }
                        }
                    } else if move_down {
                        ...
                        match app_state {
                            AppState::Menu => {
                                if selected < file_names.len().saturating_sub(1) { 
                                    let old = selected;
                                    selected += 1; 
                                    update_menu_selection(&mut display, &file_names, old, selected);
                                }
                ...
                // Actualizam ultima pozitie cunoscuta a degetului
                last_wheel_pos = Some(c_pos);
            } else {
                // A ridicat degetul de pe roata
                last_wheel_pos = None;
            }
        }
        ...
        if button.is_low() {
            ...
            if button.is_low() {
                match app_state {
                    AppState::Menu => {
                        if !file_names.is_empty() {
                            ...
                            player_cursor = 1; 
                            draw_player(&mut display, &file_names[selected], player_cursor, true);
                        }
                    }
                    AppState::Playing => {
                        match player_cursor {
                            0 => { 
                                ...
                                if len > 0 {
                                    current_idx = (current_idx + len.saturating_sub(2)) % len;
                                    FILE_DONE.store(true, Ordering::Relaxed);
                                }
                            }
                            1 => { 
                ...
                while button.is_low() { Timer::after_millis(10).await; } // Asteapta ridicarea degetului
            }
        }
        Timer::after_millis(10).await; // Loop-ul respira mult mai rapid acum (10ms in loc de 20ms/50ms)
    }
}

// ==== FUNCTII DE DESENARE INITIALA ====
...
fn draw_menu<D: DrawTarget<Color = Rgb565>>(display: &mut D, files: &Vec<ShortFileName, MAX_FILES>, selected: usize) {
    ...
    for (i, name) in files.iter().enumerate() {
        ...
        if is_selected {
            Rectangle::new(Point::new(5, y - 15), Size::new(310, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
        }
...
fn draw_player<D: DrawTarget<Color = Rgb565>>(display: &mut D, filename: &ShortFileName, cursor: usize, is_playing: bool) {
    display.clear(Rgb565::BLACK).ok();
    ...
    let labels = ["[ << ]", "[ || ]", "[ >> ]"];
    let x_positions = [10i32, 95, 190];
    
    for (i, (label, x)) in labels.iter().zip(x_positions.iter()).enumerate() {
        if i == cursor {
            Rectangle::new(Point::new(*x - 3, 185), Size::new(75, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
        }
        let color = if i == cursor { Rgb565::WHITE } else { Rgb565::CSS_STEEL_BLUE };
        let style = MonoTextStyle::new(&FONT_9X15, color);
        Text::new(label, Point::new(*x, 200), style).draw(display).ok();
    }
    update_player_status(display, is_playing);
}

// ==== FUNCTII DE PARTIAL UPDATE (RAPIDE) ====

fn update_menu_selection<D: DrawTarget<Color = Rgb565>>(
    display: &mut D, files: &Vec<ShortFileName, MAX_FILES>, old_idx: usize, new_idx: usize,
) {
    // 1. Stergem vechiul highlight (redesenam textul albastru pe fond negru)
    let y_old = 50 + old_idx as i32 * 25;
    Rectangle::new(Point::new(5, y_old - 15), Size::new(310, 22))
        .into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
    let style_old = MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_STEEL_BLUE);
    let name_str_old = core::str::from_utf8(files[old_idx].base_name()).unwrap_or("???");
    let display_str_old: String<20> = String::try_from(name_str_old).unwrap_or_default();
    Text::new(&display_str_old, Point::new(10, y_old), style_old).draw(display).ok();

    // 2. Desenam noul highlight (text alb pe fond rosu)
    let y_new = 50 + new_idx as i32 * 25;
    Rectangle::new(Point::new(5, y_new - 15), Size::new(310, 22))
        .into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
    let style_new = MonoTextStyle::new(&FONT_9X15, Rgb565::WHITE);
    let name_str_new = core::str::from_utf8(files[new_idx].base_name()).unwrap_or("???");
    let display_str_new: String<20> = String::try_from(name_str_new).unwrap_or_default();
    Text::new(&display_str_new, Point::new(10, y_new), style_new).draw(display).ok();
}
...
fn update_player_cursor<D: DrawTarget<Color = Rgb565>>(display: &mut D, old_cursor: usize, new_cursor: usize) {
    ...
    // Stergem highlight vechi
    let old_x = x_positions[old_cursor];
    Rectangle::new(Point::new(old_x - 3, 185), Size::new(75, 22))
        .into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
    let style_old = MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_STEEL_BLUE);
    Text::new(labels[old_cursor], Point::new(old_x, 200), style_old).draw(display).ok();

    // Desenam highlight nou
    let new_x = x_positions[new_cursor];
    Rectangle::new(Point::new(new_x - 3, 185), Size::new(75, 22))
        .into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
    let style_new = MonoTextStyle::new(&FONT_9X15, Rgb565::WHITE);
    Text::new(labels[new_cursor], Point::new(new_x, 200), style_new).draw(display).ok();
}

fn update_player_status<D: DrawTarget<Color = Rgb565>>(display: &mut D, is_playing: bool) {
    // Curata zona textului de status
    Rectangle::new(Point::new(80, 145), Size::new(120, 20))
        .into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
        
    let status = if is_playing { ">> Playing" } else { "|| Paused " };
    let status_style = MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_LIME_GREEN);
    Text::new(status, Point::new(80, 160), status_style).draw(display).ok();
}