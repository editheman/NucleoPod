...
// AM MARIT BUFFER-UL LA 45KB (poza 120x120 24bit ocupa ~43.200 bytes)
...
async fn main(_spawner: Spawner) {
    ...
    loop {
        ...
        if let Some(ref file) = current_file {
            ...
            // Cand melodia se termina (Auto-Next)
            if FILE_DONE.load(Ordering::Relaxed) {
                ...
                // === INCARCAM IMAGINEA BMP DACA EXISTA (AUTO-NEXT) ===
                let target_base = file_names[current_idx].base_name();
                unsafe { CURRENT_BMP_LEN = 0; }
                
                let mut found_bmp: Option<ShortFileName> = None;
                volume_mgr.iterate_dir(root_dir, |entry| {
                    if entry.name.base_name() == target_base && entry.name.extension() == b"BMP" {
                        found_bmp = Some(entry.name.clone());
                    }
                }).unwrap();
                ...
                match app_state {
                    AppState::Menu => {
                        if !file_names.is_empty() {
                            ...
                            // === INCARCAM IMAGINEA BMP DACA EXISTA (MANUAL PLAY) ===
                            let target_base = file_names[selected].base_name();
                            info!("--- DEBUG: Cautam cover pentru {=[u8]:a} ---", target_base);
                            unsafe { CURRENT_BMP_LEN = 0; }
                            ...
                            volume_mgr.iterate_dir(root_dir, |entry| {
                                let ext = entry.name.extension();
                                if ext == b"BMP" {
                                    info!("  -> Am gasit fisier BMP pe card: {=[u8]:a}.BMP", entry.name.base_name());
                                    if entry.name.base_name() == target_base {
                                        info!("  -> Meci PERFECT! Il salvam.");
                                        found_bmp = Some(entry.name.clone());
                                    }
                                }
                            }).unwrap();
                            ...
                            if let Some(bmp_name) = found_bmp {
                                info!("Incerc sa deschid fisierul BMP de pe card...");
                                if let Ok(f) = volume_mgr.open_file_in_dir(root_dir, &bmp_name, embedded_sdmmc::Mode::ReadOnly) {
                                    unsafe { 
                                        CURRENT_BMP_LEN = volume_mgr.read(f, &mut *core::ptr::addr_of_mut!(IMAGE_BUF)).unwrap_or(0); 
                                        // COPIEM valoarea intr-o variabila locala sigura pentru a o putea printa!
                                        let bytes_cititi = CURRENT_BMP_LEN; 
                                        info!("SUCCES: Am citit {} bytes din poza", bytes_cititi);
                                    }
                                    volume_mgr.close_file(f).ok();
                                } else {
                                    info!("EROARE: Nu s-a putut deschide fisierul BMP!");
                                }
                            } else {
                                info!("Nu s-a gasit nicio poza cu numele acesta :(");
                            }
                    ...
                    AppState::Playing => {
                        match player_cursor {
                            ...
                            2 => { FILE_DONE.store(true, Ordering::Relaxed); } // Next
                            3 => { // BACK TO MENU
...
fn draw_player<D: DrawTarget<Color = Rgb565>>(display: &mut D, filename: &ShortFileName, cursor: usize, is_playing: bool, bmp_data: &[u8]) {
    ...
    // --- DRAW CONTROLS (Coborate cu +20 pe axa Y) ---
    let labels = ["[ << ]", "[ || ]", "[ >> ]"];
    let x_positions = [19i32, 93, 167]; 

    for (i, (label, x)) in labels.iter().zip(x_positions.iter()).enumerate() {
        if i == cursor {
            // Y a crescut de la 175 la 195
            Rectangle::new(Point::new(*x - 3, 195), Size::new(60, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
        }
        let color = if i == cursor { Rgb565::WHITE } else { Rgb565::CSS_STEEL_BLUE };
        // Y a crescut de la 190 la 210
        Text::new(label, Point::new(*x, 210), MonoTextStyle::new(&FONT_9X15, color)).draw(display).ok();
    }

    if cursor == 3 {
        // Y a crescut de la 215 la 235
        Rectangle::new(Point::new(81, 235), Size::new(78, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
    }
    let back_color = if cursor == 3 { Rgb565::WHITE } else { Rgb565::CSS_STEEL_BLUE };
    // Y a crescut de la 230 la 250
    Text::new("[ BACK ]", Point::new(84, 250), MonoTextStyle::new(&FONT_9X15, back_color)).draw(display).ok();
...
fn update_player_cursor<D: DrawTarget<Color = Rgb565>>(display: &mut D, old_cursor: usize, new_cursor: usize) {
    ...
    // Curăță vechiul cursor
    if old_cursor < 3 {
        let x = x_positions[old_cursor];
        // Y a crescut de la 175 la 195
        Rectangle::new(Point::new(x - 3, 195), Size::new(60, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
        // Y a crescut de la 190 la 210
        Text::new(labels[old_cursor], Point::new(x, 210), MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_STEEL_BLUE)).draw(display).ok();
    } else {
        // Y a crescut de la 215 la 235
        Rectangle::new(Point::new(81, 235), Size::new(78, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
        // Y a crescut de la 230 la 250
        Text::new("[ BACK ]", Point::new(84, 250), MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_STEEL_BLUE)).draw(display).ok();
    }

    // Desenează noul cursor
    if new_cursor < 3 {
        let x = x_positions[new_cursor];
        // Y a crescut de la 175 la 195
        Rectangle::new(Point::new(x - 3, 195), Size::new(60, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
        // Y a crescut de la 190 la 210
        Text::new(labels[new_cursor], Point::new(x, 210), MonoTextStyle::new(&FONT_9X15, Rgb565::WHITE)).draw(display).ok();
    } else {
        // Y a crescut de la 215 la 235
        Rectangle::new(Point::new(81, 235), Size::new(78, 22)).into_styled(PrimitiveStyle::with_fill(Rgb565::CSS_INDIAN_RED)).draw(display).ok();
        // Y a crescut de la 230 la 250
        Text::new("[ BACK ]", Point::new(84, 250), MonoTextStyle::new(&FONT_9X15, Rgb565::WHITE)).draw(display).ok();
    }
}


fn update_player_status<D: DrawTarget<Color = Rgb565>>(display: &mut D, is_playing: bool) {
    // Curata zona din dreapta jos (langa poza) si redeseneaza statusul
    Rectangle::new(Point::new(150, 105), Size::new(120, 20)).into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK)).draw(display).ok();
    let status = if is_playing { ">> Playing" } else { "|| Paused " };
    Text::new(status, Point::new(150, 120), MonoTextStyle::new(&FONT_9X15, Rgb565::CSS_LIME_GREEN)).draw(display).ok();
}
